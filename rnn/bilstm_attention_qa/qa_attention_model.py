...
class Attention_LSTM(object):
    """带注意力机制的双向LSTM问答模型
    """
    def __init__(self, batch_size, lstm_size=500, query_steps=12, answer_steps=300, embedding_size=256,
    ...
    def inference(self, next_element):
        """forward propagation
        """
        query_vec = next_element["query"]
        answer_vec = next_element["answer"]
        if self.is_train:
            neg_answer_vec = tf.concat([tf.slice(answer_vec, [1, 0, 0], [-1, -1, -1]), [answer_vec[0]]], 0)

        # set tensor shape
        query_vec.set_shape([None, self.query_steps, self.embedding_size])
        answer_vec.set_shape([None, self.answer_steps, self.embedding_size])
        if self.is_train:
        ...
        with tf.name_scope("dropout"):
            drop_q = tf.nn.dropout(question_feat, self.dropout_keep_prob)
    ...
    def loss(self, pos_sim, neg_sim):
        ...
        margin = tf.constant(0.3, dtype=tf.float32)
    ...
    def train(self):
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                train_iterator = data_helper.get_iterator(self.train_data_file, self.batch_size)
                valid_iterator = data_helper.get_iterator(self.valid_data_file, self.batch_size)
